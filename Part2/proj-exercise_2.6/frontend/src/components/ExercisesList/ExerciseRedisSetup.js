  ...
  testConnection = async () => {
    ...
    try {
      ...
      if (err.response && err.response.status === 404) {
        error =
          "Request sent to unknown address, 404. Check network tab where the request was sent.";
      }
      if (err.message === "Not pong") {
        error =
          "Response was not what was expected. Check network tab where the request was sent.";
      }
      if (err.message === "Too slow") {
        error =
          "Response was correct but it was slow, did you set up the cache correctly?";
      }
  ...
  render() {
    return (
      <div>
        ...
        <Button
          color={this.state.success ? "green" : "orange"}
