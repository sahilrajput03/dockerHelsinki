...
app.use((req, res, next) => {
  log('#mylog#:', req.connection.remoteAddress)
  log('#mylog{when behind proxy}#:', req.headers['x-forwarded-for'])
  log('#mylog{FRONT_URL}#', FRONT_URL)
...
const reallySlowApi = async () =>
  new Promise((resolve, reject) => setTimeout(() => resolve('pong'), 11 * 1000))
...
app.get('/api*', (req, res) => {
  const hint =
    'You are probably doing 2.8 and this path starts with /api. This backend serves requests sent to "/", for example "/ping". See hint at the end of 2.8 how to remove the path prefix "/api".'
  console.log(hint)
  res.send(hint)
})
